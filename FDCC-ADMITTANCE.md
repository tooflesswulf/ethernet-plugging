...
| Clunk at every scripted transition | (a) `MotionStep` finished inside tolerance while its target was still decelerating (27 → 0 mm/s in one tick); (b) a deliberate target jump (bumpless rescale) read as velocity by the finite-difference feedforward (80 mm/s kick); (c) the stale teleop target walked back to the arm at the speed limit, fed forward | finish only after the profile ends; `Impedance.note_target_jump()` after any deliberate jump; re-anchor the leash at scripted starts (`env.reanchor()`) |
